...
import { Form } from "@/shared/ui/design/ui/form"
...
const props = defineProps<{
  ...
  asyncSubmit?: (val: GenericObject) => Promise<any>
}>()
...
const formComponentProps = computed(() => {
  if (props.form) {
    return {
      onSubmit: props.form.handleSubmit(async (val) => {
        if (props.asyncSubmit) {
          await Promise.resolve(props.asyncSubmit(val))
        }
        emits("submit", val)
      }),
    }
